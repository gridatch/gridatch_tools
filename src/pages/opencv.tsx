...
function findBestContour(src: cv.Mat, point?: cv.Point) {
  ...
  for (let i = 0; i < contours.size(); i++) {
    ...
    const inside = cv.pointPolygonTest(contour, new cv.Point(cx, cy), false);
    if (inside >= 0 && area > maxArea) {
      bestContour?.delete();
      bestContour = contour;
      maxArea = area;
    } else {
...
function warpQuadToRect(src: cv.Mat, srcQuad: cv.Mat, dstSize: cv.Size): cv.Mat {
  ...
  cv.warpPerspective(src, dst, M, new cv.Size(dstSize.width, dstSize.height));
  dstQuad.delete();
  M.delete();
  return dst;
}
 ...
 * @returns 白かどうか
 */
function isWhiteTileFromMat(mat: cv.Mat) {
  const hsv = new cv.Mat();
  cv.cvtColor(mat, hsv, cv.COLOR_RGB2HSV_FULL);
  const [h, s, v] = cv.mean(hsv);
  hsv.delete();
  // 非魂牌：彩度小、明度大
  if (s < 50 && v > 200) return true;
  // 魂牌：彩度やや小、暖色系
  if (s < 100 && h < 60) return true;
  return false;
}
...
function splitWallAndMatch(wallMat: cv.Mat): MatchingResult[] {
  ...
  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < cols; col++) {
      ...
      if (variance < 1000) {
        // 分散が少ない牌は単色牌
        if (isWhiteTileFromMat(colorRoiCenter)) {
          result.push({ tile: "P", isRed: false, score: 1 });
        } else {
          result.push({ tile: "closed", isRed: false, score: 1 });
        }
...
const MahjongRecognizer: React.FC = () => {
  ...
  const canvasRefs = {
    ...
    _4contour: useRef<HTMLCanvasElement>(null),
    _5croppedContour: useRef<HTMLCanvasElement>(null),
    _6hough: useRef<HTMLCanvasElement>(null),
    _7cluster: useRef<HTMLCanvasElement>(null),
    _8warped: useRef<HTMLCanvasElement>(null),
    _9tiles: useRef<HTMLCanvasElement[]>([]),
  };
  const tileRefs = useRef<HTMLSpanElement[]>([]);
  
  // 初期レンダリング時に空配列をセット
  useEffect(() => {
    canvasRefs._9tiles.current = Array(36).fill(null);
  ...
  const processWallImage = useCallback((img: HTMLImageElement) => {
    ...
    drawMatWithContourToCanvas(canvasRefs._4contour, colorMat, contourMat);
    ...
    drawMatToCanvas(canvasRefs._5croppedContour, binaryContourMat);
    ...
    drawMatWithLinesToCanvas(canvasRefs._6hough, colorMat, lines);
    ...
    drawMatWithPolygonToCanvas(canvasRefs._7cluster, colorMat, quad);
    ...
    drawMatToCanvas(canvasRefs._8warped, wallMat);

    drawTilesToCanvases(canvasRefs._9tiles, wallMat)
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 16, marginTop: 16 }}>
        ...
        <div><p>4. Contour</p><canvas ref={canvasRefs._4contour} style={{ maxWidth: '100%', border: '1px solid #ccc' }} /></div>
        <div><p>5. Cropped contour</p><canvas ref={canvasRefs._5croppedContour} style={{ maxWidth: '100%', border: '1px solid #ccc' }} /></div>
        <div><p>6. Hough</p><canvas ref={canvasRefs._6hough} style={{ maxWidth: '100%', border: '1px solid #ccc' }} /></div>
        <div><p>7. Cluster</p><canvas ref={canvasRefs._7cluster} style={{ maxWidth: '100%', border: '1px solid #ccc' }} /></div>
        <div><p>8. Warped</p><canvas ref={canvasRefs._8warped} style={{ maxWidth: '100%', border: '1px solid #ccc' }} /></div>
        <div>
          <p>9. Tiles</p>
          <div style={{ display: 'inline-grid', gridTemplateColumns: 'repeat(9, 1fr)', gap: 8, marginTop: 16 }}>
            {Array.from({ length: 36 }).map((_, idx) => (
              <div key={`roi-${idx}`}>
                <canvas
                  ref={el => { if (el) canvasRefs._9tiles.current[idx] = el }}
